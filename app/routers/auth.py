...
from app.services.users_service import get_user_by_email, get_current_user
...
async def send_token(form_data: Annotated[OAuth2PasswordRequestForm, Depends()]):
    ...
    return await create_access_token(user_data=form_data)
...
async def register_user(user_data: users.UserCreate):
    if await get_user_by_email(email=user_data.email):
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="User already exists")
    user = await register(user_data=user_data)
    return user
